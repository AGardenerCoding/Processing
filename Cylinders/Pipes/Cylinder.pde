 ...
 *  2-9-22
 *
 *  A class to draw a cylinder between two 3d points, with options to add one or both end caps.
 *
 *  'pipes' by w.j.baker
 *
...
class Cylinder
{
    ...
    PVector end1,           // End points of the central cylinder axis.
            end2,
            axis,           // NORMALIZED vector representing the direction of central axis, STARTING AT ORIGIN.
            offset1,
            offset2;
                     ...
                     boolean addCap1, boolean addCap2, int cap1Type, int cap2Type )
    {
        ...
        axis = PVector.sub( end2, end1 );
        length = axis.mag();    // original length before normalization
        axis.normalize();
    ...
    void calcCylinderVerts()
    {
        ...
        //tube.stroke( pipeStrokeClr );
        tube.noStroke();
        tube.fill( pipeFillClr );
    ...
    void changeLength( float dist )
    {
        axis.setMag( dist );
        end2.add( axis );

        // Reset axis as a vector the length and direction of end1->end2
        axis = PVector.sub( end2, end1 );

        // Recalculate the cylinder's length.
        length = axis.mag();
    ...
    /* Because the cross product of end1 and end2 is used to calculate the offset for the end rings, neither end1
     * nor end2 can have all three coordinates equal to 0. Also, the cross product won't work if the x-coord of
     * either vector is non-zero with y and z coords of both equal to 0. If either of these is the case, then modify
     * the z-coord of end1 to have a very small value.
     */

    void checkCoordinates()
    {
        if ( end1.x == 0 && end1.y == 0 && end1.z == 0 )
        {
            end1.z = 0.000001f;
        }

        if ( end2.x == 0 && end2.y == 0 && end2.z == 0 )
        {
            end2.z = 0.000001f;
        }

        if ( end1.y == 0 && end1.z == 0 && end2.y == 0 && end2.z == 0 )
        {
            end1.z = 0.000001f;
        }
